package com.mobile.pocketrivals.ui.theme

import androidx.compose.ui.unit.dp
import androidx.compose.ui.unit.sp

object Dimensions {
  // Padding
  val SmallPadding = 4.dp
  val MediumPadding = 8.dp
  val LargePadding = 12.dp
  val ExtraLargePadding = 16.dp

  // Spacing
  val SmallSpacer = 8.dp
  val MediumSpacer = 12.dp
  val LargeSpacer = 40.dp
  val ExtraLargeSpacer = 32.dp

  val CardElevation = 2.dp

  // Round Corner
  val SmallRoundedCorner = 4.dp
  val MediumRoundedCorner = 8.dp

  // Icon Size
  val SmallIcon = 14.dp
  val MediumIcon = 50.dp

  // Image Size
  val SmallImage = 50.dp
  val MediumImage = 120.dp
  val LargeImage = 150.dp

  // Font Size
  val SmallFontSize = 14.sp
  val MediumFontSize = 16.sp
  val LargeFontSize = 24.sp

  //  Weight
  val SmallWeight = 0.5f
  val MediumWeight = 1f

  // Divider Thickness
  val SmallDividerThickness = 1.dp
  val MediumDividerThickness = 2.dp

  // Info Row Width
  val InfoRowWidth = 120.dp

  // Search Height
  val SearchBarHeight = 56.dp

  // Elevation
  val Elevation = 4.dp

  // Card Size
  val SmallCard = 100.dp
  val MediumCard = 120.dp
  val LargeCard = 150.dp

  // Patch Notes Card
  val PatchCardHeight = 230.dp

  // Carousel
  val CarouselHeight = 250.dp

  // Loader size
  val LoaderSize = 75.dp
}
